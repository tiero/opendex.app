...
import { detectSwap, constructClaimTransaction, ContractABIs } from 'boltz-core';
...
import { SwapUpdateEvent } from '../../constants/environment';
import { LocalStorageState } from '../../constants/environment';
import { formatTokenAmount } from '../../utils/ethereumDecimals';
import { getHexBuffer, getNetwork, broadcastRefund as broadcastClaimTransaction } from '../refund/refundDuck';
...
const currentReverseState = activeSwap === 'reverse' ? JSON.parse(localStorage.getItem('currentReverseState')) : {};
...
export const reverseReducer = (state = initialState, action) => {
    switch (action.type) {
        case SET_SWAP_DETAILS:
            localStorage.setItem(LocalStorageState.CurrentReverseState, JSON.stringify({
              ...state,
...
const getClaimTransaction = (onchainCurrency, swapInfo, response, feeEstimation) => {
...
export const claimSwap = async (dispatch, nextStage, signer, swapInfo, swapResponse) => {
  const submarineState = JSON.parse(localStorage.getItem(LocalStorageState.CurrentSubmarineState));
  ...
  if (receiveCurrency.type !== CurrencyTypes.BitcoinLike) {
    ...
    if (receiveCurrency.type === CurrencyTypes.Ether) {
      const etherSwap = new Contract(contracts.swapContracts.EtherSwap, ContractABIs.EtherSwap, signer);
      ...
      const tokenContract = new Contract(contracts.tokens[receiveCurrency.symbol], ContractABIs.ERC20, signer);
      const erc20Swap = new Contract(contracts.swapContracts.ERC20Swap, ContractABIs.ERC20Swap, signer);

      try {
        const claimTransaction = await erc20Swap.claim(
          getHexBuffer(swapInfo.preImage),
          formatTokenAmount(swapDetails.onchainAmount, await tokenContract.decimals()),
...
export const handleReverseSwapStatus = ({ instantSwap, privateKey, address , preImage }, signer) => (
    ...
    switch (status) {
      ...
      case SwapUpdateEvent.MinerFeePaid:
        dispatch(
          setEthereumPrepayMinerFeePaid(true),
        );
        break;
...
export const createReverseSwap = (dispatch, payload, nextStep, extraDetails, signer) => {
    createSwap(dispatch, payload, (data) => {
        dispatch(setSwapDetails(data));
        nextStep();
        localStorage.setItem(LocalStorageState.ActiveSwap, 'reverse');
    }, nextStep, handleReverseSwapStatus(extraDetails, signer));
}
