...
const EthereumAccount = props => {
  ...
  useEffect(() => {
    const gasCheck = async () => {
      ...
      if (isEther) {
        gasNeeded = isReverseSwap ? gasUsage.EtherSwap.claim : gasUsage.EtherSwap.lock;
      } else {
        gasNeeded = isReverseSwap ? gasUsage.ERC20Swap.claim : gasUsage.ERC20Swap.lock;
      }
    ...
    <div className={classes.ethereumWrapper}>
        ...
        <div className={classes.accountInfoWrapper}>
            ...
            <div className={classes.accountInfo}>
                <span className={classes.accountValue}>{account.substring(0, 6)}...{account.slice(-4)}</span>
                <a className={classes.viewOnEtherscanLink} href={`${explorers.ethereum.address}${account}`} target="_blank" rel="noopener noreferrer">
                  <img src={svgIcons.externalLinkIcon} alt='external-link' />
                  View on Etherscan
                </a>
            </div>
        </div>
        {error && <div className={classes.errorLabel}>
          You do not have enough Ether to pay for the miner fee required claim the USDT of the Swap.<br />
