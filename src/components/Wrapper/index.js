...
const Wrapper = (props) => {
    ...
    const drawerHandler = useCallback((nextState) => () => {
        setDrawer(nextState);
    }, []);
    ...
    const getDrawerComponent = () => (
        <DrawerContext.Provider value={{
            ...
            >
                <div
                    className={`drawer-wrapper__content ${classes.fullList} ${classes.content} ${openDrawer ? classes.contentShift : ''}`}
                    role="presentation"
                >
    ...
    if(isMobileView) {
        return (<div className={`homepage-drawer ${openDrawer ? 'open' : ''}`}>
            {!openDrawer && <div
                className="drawer-options"
                role="presentation"
                onClick={drawerHandler(true)}
            >
                <button onClick={drawerHandler(true)} className="drawer-btn"></button>
    ...
    return (<div className="homepage-modal max-width-margin-right">
        {props.children}
    </div>);
}
