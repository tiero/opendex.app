...
function checkStatus(response) {
    if (response.status >= 200 && response.status < 300 && response.data && response.data.status !== 'FAIL') {
      return response;
    }
...
function request(url, options = {}) {
    ...
    const axiosRequest = axios(requestUrl, { ...options, cancelToken: source.token, validateStatus: () => true })
      .then(checkStatus)
      .then(extractData);
