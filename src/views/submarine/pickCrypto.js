...
const PickCrypto = props => {
  ...
  const onSendCurrencyChange = ({ target: { value: val } }) => {
    ...
    const newReceiveValue =
      Number(Number(value * effectiveRate).toFixed(8)) -
  ...
  const onReceiveCurrencyChange = ({ target: { value: val } }) => {
    ...
    const value = Number(val);
    const newSendValue =
      Number(Number(value / effectiveRate).toFixed(8)) + Number(minerFees());
  ...
  */
  ...
  const connectEthereumWallet = async () => {
    ...
    if (
      ...
      setError(
        `Ethereum wallet is not on network ${
          backendEthereumNetwork.name ||
    ...
    switch (sendCurrencyType.swapValues.type) {
      ...
      case CurrencyTypes.ERC20:
        const contractAddress =
          contractAddresses.ethereum.tokens[sendCurrencyType.symbol];
  ...
  if (account) {
    const isEther =
      sendCurrencyType.swapValues.type === CurrencyTypes.Ether ||
    ...
    return (
      <EthereumAccount
        ...
        etherSendAmount={
          isSendCurrencyEthereum && isEther ? Number(sendCurrencyValue) : 0
        }
