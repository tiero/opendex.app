...
const Send = (props) => {
    ...
    if (signer !== undefined) {
        swapContract = sendCurrency.type === CurrencyTypes.Ether ?
            new Contract(contractAddresses.ethereum.swapContracts.EtherSwap, ContractABIs.EtherSwap, signer) :
            new Contract(contractAddresses.ethereum.swapContracts.ERC20Swap, ContractABIs.ERC20Swap, signer);
    
        tokenContract = sendCurrency.type === CurrencyTypes.ERC20 ?
            new Contract(contractAddresses.ethereum.tokens[sendCurrency.symbol], ContractABIs.ERC20, signer) :
            undefined;
    }
    ...
    useEffect(() => {
        ...
        const checkApproval = async () => {
            ...
            if (allowance.lt(formatTokenAmount(swapDetails.expectedAmount, decimals))) {
    ...
    const approveFunds = () => {
        ...
        tokenContract.approve(swapContract.address, constants.MaxUint256).then((reciept) => {
            reciept.wait(1).then(() => {
                setFundsApproved(true);
            })
        });
    ...
    const sendFunds = async () => {
        ...
        const preimageHash = Buffer.from(decodeInvoice(swapInvoice).paymentHash, 'hex');
    ...
    >
        ...
        {sendCurrency.type !== CurrencyTypes.BitcoinLike ? 'to initiate the swap' : 'to this address'}
        {feeEstimations && <div className={classes.feeEstimationWrapper}>
            <span className={classes.feeEstimationLabel}>{feeEstimations[sendCurrency.symbol]} sat/vbyte</span>
            <TextInfo explanation={true} onMouseEnterHandler={handleFeeEstimationPopoverOpen} onMouseLeaveHandler={handleFeeEstimationPopoverClose} />
        </div>}
    </Typography>);

    const renderAddressFields = () => (<div className="submarine__send_invoice_group" style={{marginTop: 0}}>
        <DrawQrCode
            size={200}
            link={qrCode}
        />
        <div>
            {!isCopied && <div className="textinfo-label">Copy the {sendCurrency.label} address</div>}
    ...
    return (<div className="submarine__send">
        {renderHeader()}
        {!isEthereum && renderAddressFields()}
        <div className={`submarine__send_footer ${isEthereum ? 'submarine__send_ethereum_footer' : ''}`}>
            <Divider className={(sendCurrency.type === CurrencyTypes.BitcoinLike && !isMobileView) ? classes.customDivider : ''} />
            {
                isChannelCreation && <a className='explorer-link' href={
                        getBlockExplorerTransactionLink(sendCurrency.symbol, channelCreation.fundingTransactionId)
                    } target="_blank" rel="noopener noreferrer">
                    <img src={svgIcons.externalLinkIcon} alt='external-link' />
                    Channel funding transaction
                </a>
            }
            {isEthereum && !fundsSent && <div className={showApproveButton ? classes.ethereumButtons : ''}>
                ...
                <Button
                    ...
                    className={`${swapStatus.error ? 'failure-button' : 'next-step-button'} waiting button-with-spinner`}
                >
                    {fundsSent === false ? 'Waiting for one confirmation' : 'Send funds'}
        ...
        <Popover id='fee-estimation-popover' open={feeEstimationPopoverOpen} anchorEl={feeEstimationAnchorEl} onCloseHandler={handleFeeEstimationPopoverClose} text='Minimum recommended fee to get your transaction confirmed quickly.' />
    </div>);
};
