...
import { Typography, Button, Divider, TextareaAutosize, Switch, FormControlLabel } from '@material-ui/core';
...
import { updateInvoice, createSwap } from '../../services/submarine/submarineDuck';
...
import {
    ...
    selectOrderSide,
    selectSwapDetails,
    selectReceiveCurrency, selectSendCurrency,
...
const Destination = (props) => {
    ...
    useEffect(() => {
        if (
          ...
          sendCurrency.type === CurrencyTypes.BitcoinLike
            ...
            setKeys(generateKeys(receiveCurrency.symbol === 'BTC' ? bitcoinNetwork : litecoinNetwork));
        }
    ...
    const addToLocalStorage = (data) => {
        ...
        if (sendCurrency.type === CurrencyTypes.BitcoinLike) {
            ...
            localStorage.setItem('swapIds', JSON.stringify([
              ...(lastSwap ? [lastSwap] : []),
              {
                id: data.id,
    ...
    const renderInvoiceFields = () => (<div className="submarine__destination_invoice_group">
        ...
        />
        {!!invoice && !isInvoiceValid(invoice) && <div className={'error-msg'}>Invalid invoice</div>}
    </div>);

    return (<div className="submarine__destination">
        {renderHeader()}
        {renderInvoiceFields()}
            ...
            <FormControlLabel
                ...
                label={
                    <div>
                        ...
                        <img src={svgIcons.questionIcon} alt='question-icon' className='question-icon' onMouseEnter={handleRefundFilePopoverOpen} onMouseLeave={handleRefundFilePopoverClose} />
                    </div>
                }
           />
        ...
        />}
        <Popover id='invoice-popover' open={invoiceOpen} anchorEl={invoiceAnchorEl} onCloseHandler={handleInvoicePopoverClose} text='A lightning invoice is how you receive payments on the lightning network. Please use a Lightning wallet to create an invoice.' />
        <Popover id='refund-file-popover' open={refundFileOpen} anchorEl={refundFileAnchorEl} onCloseHandler={handleRefundFilePopoverClose} text="The refund file is stored in your browser. Downloading it means you won't have a single point of failure for saved refund information, which is required for keeping the swap trustless." />
