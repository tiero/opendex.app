...
const Destination = (props) => {
    ...
    const sendAmount = Math.round(parseFloat(sendCurrency.amount) * satConversionValue);
    ...
    const nextStepHandler = () => {
        ...
        localStorage.setItem(LocalStorageState.ExtraDetails, JSON.stringify({
            pairId,
            preImage,
            instantSwap,
            address: address,
    ...
    const renderInvoiceFields = () => (<div className="submarine__destination_invoice_group">
        <div className="invoice-header">
            <div>
                {receiveCurrency.label} receiving address
            </div>
        </div>
        <TextareaAutosize
        ...
        />
        {!!address && !isAddressValid(address, currencyNetwork) && <div className={'error-msg'}>Invalid address</div>}
    </div>);
    ...
    return (<div className="submarine__destination">
        ...
        <div className="submarine__destination_invoice_footer">
            ...
            <FormControlLabel
                ...
                label={
                    <div>
                        Swap Instantly
                        <img src={svgIcons.questionIcon} alt='question-icon' className='question-icon' onMouseEnter={handleInstantSwapPopoverOpen} onMouseLeave={handleInstantSwapPopoverClose} />
                    </div>
                }
           />
        ...
        <Popover id='instant-swap-popover' open={instantSwapOpen} anchorEl={instantSwapAnchorEl} onCloseHandler={handleInstantSwapPopoverClose} text="Enabling swap instantly means you agree to accept a 0-conf transaction from Boltz, which results in an instant swap." />
    </div>);
};
