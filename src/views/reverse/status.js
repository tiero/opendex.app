...
import { selectSendCurrency, selectReceiveCurrency } from '../../services/submarine/submarineSelectors';
...
const Status = () => {
    ...
    return (<div className="submarine__status">
            <img src={error ? svgIcons.snap : svgIcons.greenTick} alt="" />
            {!error && <div className="message">
                You have successfully swapped
                <span className='highlight'>{sendCurrency.amount} {sendCurrency.label}</span>
                for{isMobileView ? <br /> : '' }
                <span className='highlight'>{receiveCurrency.amount} {receiveCurrency.label}</span>
            </div>}
            <a className="explorer-link" href={
                    // The user received the currency on which the claim transaction was sent
                    getBlockExplorerTransactionLink(receiveCurrency.symbol, claimTransactionId)
