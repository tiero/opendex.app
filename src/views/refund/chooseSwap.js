...
import { selectFileStatus, selectFileDetails } from '../../services/refund/refundSelectors';
import {
    setEta,
    checkStatus,
    setID as setSwapID,
...
const ChooseSwap = (props) => {
    ...
    useEffect(() => {
        ...
        if (signer !== undefined) {
            abortQueryingEthereumSwaps = queryEthereumSwaps(signer, contractAddresses, (ethereumSwapID) => {
    ...
    const nextStepHandler = () => {
        ...
        if (id) {
            ...
            if (swapDetails.type === CurrencyTypes.Ether || swapDetails.type === CurrencyTypes.ERC20) {
    ...
    >
        ...
        >
            If your on-chain to lightning swap didn't succeed for some reason, please upload the refund file you downloaded to check status and start the refunding process.
        </Typography>
    </Typography>);
    ...
    const renderSwapFields = () => (<div className="refund__choose-swap-fields">
        <div className="recent-swaps">
            ...
            >
                ...
                <img src={svgIcons.questionIcon} alt='question-icon' className='question-icon' onMouseEnter={handleRecentSwapsPopoverOpen} onMouseLeave={handleRecentSwapsPopoverClose} />
            </Typography>
            {!swapIDs?.length && <span className={'info'}>No recent swaps found...</span>}
            {!!swapIDs?.length && <ul className="list">
                {swapIDs.map(({ id: swapID, date }) => (<li>
                    <Button
                        onClick={idSelectionHandler(swapID)}
                        className={`${id === swapID ? 'selected' : ''}`}
                    >
                        <span>ID: {swapID}</span>
                        <span>{diff_minutes(new Date(), typeof date === 'string' ? new Date(date) : date)} min ago</span>
                    </Button>
                </li>))}
            </ul>}
        </div>
    ...
    return (<div className="refund__choose-swap">
        {renderHeader()}
        {renderSwapFields()}
            ...
            <Button
                ...
                onClick={async () => {
                    ...
                    if (
                        ...
                        const queriedContracts = await (await fetch(GET_CONTRACTS)).json();
        ...
        <Popover id='recent-swaps-popover' open={recentSwapsOpen} anchorEl={recentSwapsAnchorEl} onCloseHandler={handleRecentSwapsPopoverClose} text="These are the refund files that are stored in the browser. Clearing your browser cache would result in the deletion of these files." />
    </div>);
};
